...
@php
    if (!session()->has('tab')) $currTab = 'session';
    else $currTab = session()->get('tab');
@endphp
    ...
    <div class="flex flex-col justify-center content-center w-full bg-slate-400 p-3 rounded-md mt-4">
        <div role="tablist" class="tabs tabs-lifted">
            ...
            <input
                ...
                name="my_tabs_2"
                role="tab"
                class="tab bg-slate-500 font-medium text-slate-50"
                aria-label="Session"
                {{ ($currTab == 'session') ? 'checked' : '' }}
            />
                            ...
                            <tr class="text-slate-900 font-medium" style="font-size: 0.9rem;">
                                ...
                                <td width="20%" class="text-center py-5 text-white">
                                    @php
                                        ...
                                        if ($session->open <= \Illuminate\Support\Carbon::now() && $session->close >= \Illuminate\Support\Carbon::now()) {
                                            $status = "active";
                                            $badge = "bg-green-900";
                                        }
                                    @endphp
            ...
            <input
                ...
                class="tab bg-slate-500 font-medium text-slate-50"
                aria-label="Alpha"
                {{ ($currTab == 'alpha') ? 'checked' : '' }}
            />
            <div role="tabpanel" class="tab-content bg-slate-500 rounded p-6">
                <p class="text-xl">Current Health: <span class="font-bold">{{ ($alpha) ? number_format($alpha->health) : 0 }}</span></p>
                <form action="{{ route('super-si.gamebesar.alpha.update') }}" method="POST">
                    ...
                    <label class="form-control w-full">
                        ...
                        <input
                            ...
                            name="health"
                            class="input bg-slate-600 w-full text-slate-800 font-semibold border-slate-600"
                            id="editName"
                            required
                        />
                    </label>
                    <button class="w-full bg-green-600 text-slate-50 font-semibold mt-4 py-2.5 px-4 rounded-lg select-none hover:bg-green-500 active:scale-95 transition-all"
                    >
                        Update
                    </button>
                </form>
